...
def process_clip(model, preprocess, device, root: Path, media_path: Path, kf_root: Path, max_frames: int = 12) -> Tuple[ClipMeta, np.ndarray]:
    ...
    frames = extract_keyframes(media_path, kf_dir, max_frames=max_frames, size=224)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Where to write outputs (parquet, vectors.npy, id_index.json, keyframes/)",
    )
    ...
    parser.add_argument("--keyframes", type=int, default=12, help="Frames per clip for embeddings (3–16)")
    ...
    for path in tqdm(media_files, desc="Ingesting"):
        try:
            meta, emb = process_clip(model, preprocess, device, assets_dir, path, kf_root, max_frames=args.keyframes)
