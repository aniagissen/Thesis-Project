...
"""
import argparse, json, time, uuid, re, math
...
def set_clip_text(node: Dict[str, Any], text: str) -> None:
    ...
    def _set_vhs_fps(workflow: Dict[str, Any], fps: int) -> None:
        try:
        ...
        if isinstance(w, dict):
            if "frame_rate" in w:
                w["frame_rate"] = int(fps)

    def _set_frame_counts_heuristic(workflow: Dict[str, Any], frames: int) -> int:
        """Try to set frame counts on common params: frame_count, num_frames, frames."""
        hits = 0
        for n in workflow.get("nodes", []):
            w = n.get("widgets_values")
            # widgets could be dicts or lists; we only touch dicts with known keys
            if isinstance(w, dict):
                for k in list(w.keys()):
                    kl = str(k).lower()
                    if kl in {"frame_count", "num_frames", "frames"}:
                        try:
                            w[k] = int(frames)
                            hits += 1
                        except Exception:
...
def wait_for_completion(server: str, prompt_id: str, poll_s: float = 2.0, timeout_s: int = 3600) -> Dict[str, Any]:
    ...
    def _load_plan_items(path: Path) -> List[dict]:
        data = json.loads(path.read_text(encoding="utf-8"))
        fps = int(data.get("fps", 12))
        acts = data.get("acts", [])
        items = []
        for a in acts:
            prompt = a.get("prompt") or a.get("generated_prompt") or ""
            if not prompt:
                continue
            items.append({
                "prompt": prompt,
        ...
        if not items:
            raise RuntimeError("No acts with prompts found in plan.json")
        return items
...
def main():
    ...
    ap.add_argument("--jsonl", required=False, type=Path, help="JSONL (or .txt) file with prompts")
    ap.add_argument("--plan", required=False, type=Path, help="Beat/act plan JSON with durations and prompts")
    ...
    ap.add_argument("--fps", type=int, default=12, help="FPS to set on video combine (and for frame calc)")
    ap.add_argument("--set-frames", action="store_true", help="Heuristically set frame_count/num_frames in workflow")
    args = ap.parse_args()
    ...
    if args.plan and args.plan.exists():
        items = _load_plan_items(args.plan)
    elif args.jsonl and args.jsonl.exists():
        items = load_jsonl(args.jsonl)
    else:
        raise SystemExit("Provide --plan plan.json or --jsonl prompts.jsonl/.txt")
    ...
    for i, item in enumerate(items, 1):
        ...
        # set fps on VHS if present
        _set_vhs_fps(wf, int(item.get("fps") or args.fps))
        # optionally set frame counts across nodes
        frames = None
        if "duration_s" in item and item["duration_s"]:
            frames = int(math.ceil(float(item["duration_s"]) * int(item.get("fps") or args.fps)))
        if args.set_frames and frames:
            _set_frame_counts_heuristic(wf, frames)
