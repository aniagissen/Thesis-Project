...
DEFAULT_SYSTEM = (
    ...
    "concise sentence describing the shot with a clear visual description "
    "of the scene. Avoid exposition and meta-notes. No numbering, no lists, "
...
# Creating sidebar with user options
    with st.sidebar:
            ...
            if gen_clicked:
                ...
                if json_path.exists() and not settings["overwrite_ok"]:
                    ...
                    record = {
                        ...
                        "style_suffix": { "enabled": bool(settings.get("enable_suffix")),  "applied": bool(applied),  "preset": settings.get("style_choice"),  "text": settings.get("suffix_text", "")},
    ...
    # Run batch prompts in ComfyUI workflow
    with st.expander("Run batch in ComfyUI"):
        sid = st.session_state.get("session_timestamp", "session_default")
        default_prefix = f"Batch_{sid}"
        # Prefer flexible timestamped runner
        ...
        run_clicked = st.button("Run batch prompts now")
        if run_clicked:
